...
import { getUserOrderDetails, clearErrors } from '../../actions/orderAction'

const OrderDetails = ({ match }) => {
    const alert = useAlert()
    const dispatch = useDispatch();

    const { loading, error, order = {} } = useSelector(state => state.userOrderDetails)
    useEffect(() => {
        dispatch(getUserOrderDetails(match.params.id));
                                ...
                                {order.orderItems && order.orderItems.map(item => (
                                    <div key={item.productId} className="row my-5">
                                        <div className="col-4 col-lg-2">
                                            <img src={item.product.photos[0].url} alt={item.product.name} height="45" width="65" />
                                        </div>
